...
namespace common
{
    ...
    {
        private static readonly BigInteger K = 1000;

        public static BigInteger ParseBig(string s, BigInteger defaultValue = default)
        {
            if (string.IsNullOrWhiteSpace(s))
                return defaultValue;
            if (!BigInteger.TryParse(s.Trim(), NumberStyles.Integer, CultureInfo.InvariantCulture, out var v))
                return defaultValue;
            return v;
        }
        ...
        public static string FormatAbbreviated(BigInteger value, int decimals = 2)
        {
            ...
            if (v < K)
                return sign + v.ToString(CultureInfo.InvariantCulture);

            var digits = v.ToString(CultureInfo.InvariantCulture);
            // digitExp: the 10^(3n) bucket this number falls in, by digit count
            var digitExp = new BigInteger(((long)digits.Length - 1) / 3 * 3);
            var scales = AllScales;
            var chosen = scales[scales.Length - 1];
            for (var i = 0; i < scales.Length; i++)
            {
                var scaleExp = scales[i].Exponent;
                var scaleExpStr = scaleExp.ToString(CultureInfo.InvariantCulture);
                bool match;
                // Case B: literal power-of-ten thresholds (HCePi…YZCePi) use digit-group matching
                bool isLiteral = scaleExpStr.Length >= 7 && !scaleExpStr.EndsWith("003", StringComparison.Ordinal);
                if (isLiteral)
                {
                    var literalGroupExp = GetLiteralScaleGroupExponent(scaleExpStr);
                    match = digitExp >= literalGroupExp;
                }
                else
                {
                    match = digitExp >= scales[i].MatchExponent; // use MatchExponent for digit-count compare
                }
                if (match)
                {
                    chosen = scales[i];
            ...
            var chosenExpStr = chosen.Exponent.ToString(CultureInfo.InvariantCulture);
            int intDigits;
            bool chosenIsLiteral = chosenExpStr.Length >= 7 && !chosenExpStr.EndsWith("003", StringComparison.Ordinal);
            if (chosenIsLiteral)
            {
                // literal power-of-ten scales map to digit-group exponent = length - 2
                intDigits = digits.Length - GetLiteralScaleGroupExponentInt(chosenExpStr);
            }
            else
            {
                // digit-count (sentinel or direct): shift = MatchExponent value
                intDigits = digits.Length - (int)chosen.MatchExponent;
            }
            if (intDigits < 1) intDigits = 1;
            if (intDigits > digits.Length) intDigits = digits.Length;

            var head = digits.Substring(0, intDigits);
            var fracLen = Math.Max(0, decimals);
            var frac = "";
            if (fracLen > 0 && intDigits < digits.Length)
            {
                var take = Math.Min(fracLen, digits.Length - intDigits);
                frac = "." + digits.Substring(intDigits, take).TrimEnd('0');
                if (frac == ".")
                    frac = "";
            }

            return sign + head + frac + chosen.Suffix;
        }

        public static string FormatAbbreviated(string decimalString, int decimals = 2)
        {
            return FormatAbbreviated(ParseBig(decimalString, BigInteger.Zero), decimals);
        }
        ...
        private static BigInteger GetAbbreviatedExponent(string s)
        {
            ...
            for (var i = 0; i < scales.Length; i++)
            {
                ...
                if (scaleExpStr.Length >= 7 && !scaleExpStr.EndsWith("003", StringComparison.Ordinal))
                    return GetLiteralScaleGroupExponent(scaleExpStr);
                // Sentinel or direct-shift: MatchExponent is the digit-count threshold
                return scales[i].MatchExponent;
            }
        ...
        public static BigInteger ParseBigWithSuffix(string s, BigInteger defaultValue = default)
        {
            ...
            if (matchedScale == null)
                return defaultValue;
            var exponent = matchedScale.Exponent;
            ...
            var matchExp = matchedScale.MatchExponent;
            var matchExpStr = matchExp.ToString(CultureInfo.InvariantCulture);
            var sortExpStr = exponent.ToString(CultureInfo.InvariantCulture);
            // Sentinel sort keys (ends in "003", 4+ digits): MatchExponent holds real digit-count
            // Direct-shift (<=6 digits) and literal threshold (7+ not ending 003): handled below
            if (matchExpStr.Length >= 4 && matchExpStr.EndsWith("003", StringComparison.Ordinal))
                return sign < 0 ? -BigInteger.Pow(10, 350) : BigInteger.Pow(10, 350);

            const int MaxSafeShift = 350;
            BigInteger result;
            var fracDigitsBi = new BigInteger(fracDigits);
            int expShiftInt;
            if (sortExpStr.Length >= 7 && !sortExpStr.EndsWith("003", StringComparison.Ordinal))
                expShiftInt = GetLiteralScaleGroupExponentInt(sortExpStr); // literal power-of-ten scales map to grouped exponent
            else
                expShiftInt = (int)matchExp;          // direct-shift or sentinel: use MatchExponent
            var expShift = new BigInteger(expShiftInt);
            if (expShift >= fracDigitsBi)
            {
                var diff = expShift - fracDigitsBi;
                var shift = diff > MaxSafeShift ? MaxSafeShift : (int)diff;
                result = sig * BigInteger.Pow(10, shift);
            }
            else
            {
        ...
        private static int GetLiteralScaleGroupExponentInt(string scaleExpStr)
        {
            // literal thresholds of the form 1 followed by zeros represent group exponents.
            // Compute as ((digitCount-1)/3)*3 to match the abbreviation formula.
            // e.g. "10000000000000000" (17 chars, 1e16) -> ((17-1)/3)*3 = 15
            var digitCount = scaleExpStr.Length;
            return Math.Max(0, ((digitCount - 1) / 3) * 3);
        }

        private static BigInteger GetLiteralScaleGroupExponent(string scaleExpStr)
        {
            return new BigInteger(GetLiteralScaleGroupExponentInt(scaleExpStr));
        }
