...
namespace common
{
    ...
    public readonly struct StringInt : IComparable<StringInt>, IEquatable<StringInt>
    {
        ...
        /// <summary>
        /// Tolerant constructor: never throws. Values that are plain decimal
        /// strings parse exactly into BigInteger; scientific / power-tower forms
        /// ("1.5e308", "1e(1e100)") keep a zero BigInteger value but preserve the
        /// ORIGINAL text, so ToString() round-trips the wire/storage format
        /// losslessly (mirrors the FNA client BigDouble tier).
        /// </summary>
        public StringInt(string value)
        {
            _value = string.IsNullOrEmpty(value) ? BigInteger.Zero : BigIntUtils.ParseBig(value, BigInteger.Zero);
            _cachedString = value;
        }
        ...
        public static bool TryParse(string s, out StringInt result)
        {
            ...
            if (!BigIntUtils.TryParseBigDouble(s, out var _))
            {
                result = Zero;
                return false;
            }
            result = new StringInt(s);
            return true;
        }
